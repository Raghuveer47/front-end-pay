...
function EmployeeDetail() {
  ...
  const handleDownloadPdf = async () => {
    try {
      const response = await axios.get(`https://payroll-backend-8avf.onrender.com${id}`, {
        responseType: 'blob',
      });
  ...
  useEffect(() => {
    axios.get(`https://payroll-backend-8avf.onrender.com${id}`)
      .then(res => {
        setEmployee(res.data.Result);
        axios.get(`https://payroll-backend-8avf.onrender.com=${res.data.Result._id}`)
          .then(res => {
  ...
  const handleLogout = () => {
    axios.get('https://payroll-backend-8avf.onrender.com')
      .then(res => {
        navigate('/start');
      })
      .catch(err => console.log(err));
